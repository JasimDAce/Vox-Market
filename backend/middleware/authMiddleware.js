...
const requireAuth = (req,res,next) =>{
    ...
    if(token){
        jwt.verify(token,process.env.JWT_SECRET,(err,decodedToken)=>{
            if(err){
               ...
               return res.status(400).json({message:"Unauthorized access"});
                ...
                Model.findById(decodedToken.id).select('-password')
                .then((result) => {
       ...
       return res.status(401).json({message:"No Token Found,authorization denied"});//giving 401 as response to check in frontend for lient side routing.
